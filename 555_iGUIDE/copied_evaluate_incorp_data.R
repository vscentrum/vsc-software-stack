...
rm(sample_index)
valid_seqnames <- names(GenomeInfoDb::seqinfo(ref_genome)); bad <- !algnmts$seqnames %in% valid_seqnames; if (any(bad)) { warning("Dropping ", sum(bad), " alignments on seqnames not in reference: ", paste(unique(algnmts$seqnames[bad]), collapse=", ")); algnmts <- algnmts[!bad, ] }
