...
class EB_CP2K(EasyBlock):
    ...
    def configure_common(self):
        ...
        # SPGLIB
        libspg = get_software_root('spglib')
        if libspg:
            options['DFLAGS'] += ' -D__SPGLIB'
            options['LIBS'] += ' -L%s/lib -lsymspg' % libspg
            options['INCS'] += ' -I%s/include' % libspg

        # DFTD4 
        libdftd4 = get_software_root('dftd4')
        if libdftd4:
            if cp2k_version < LooseVersion('2024.2'):
                raise EasyBuildError("This version of CP2K does not offer support for dftd4")

            gcc_version = get_software_version('GCCcore') or get_software_version('GCC')

            options['DFLAGS'] += ' -D__DFTD4'
            options['LIBS'] += ' -L%s/lib -ldftd4' % libdftd4
            options['INCS'] += ' -I%s/include/dftd4/GNU-%s' % (libdftd4, gcc_version)

            # also add dependencies for dftd4
            for dependency in ['mstore', 'multicharge', 'mctc-lib']:
                libdep = get_software_root(dependency)
                if not libdep:
                    raise("Dependency %s for dfdt4 missing" % dependency)
                options['LIBS'] += ' -L%slib -l%s' % (libdep, dependency)
                options['INCS'] += ' -I%s/include' % libdep
        else:
            self.log.info("dftd4 module not loaded, so building without dftd4 support")
