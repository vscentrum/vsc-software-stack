...
__device__ void bonds_gpu(const int                   i,
                          ...
                          const float4                gm_xq[],
                          gmx::ForceBufferElementType gm_f[],
                          float3                      sm_fShiftLoc[],
                          ...
                          float*                      vtotElec_loc)
{
    if (i < numBonds)
    {
        ...
        float force_14_aj = scale_factor * gm_xq[ai].w * rinv;

        /* Add the 1-4 forces (to enable constant-pH bonded on the GPU) */
        atomicAdd(&gm_f[ai].w, force_14_ai);
        atomicAdd(&gm_f[aj].w, force_14_aj);
