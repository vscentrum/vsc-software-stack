...
class EB_Amber(CMakeMake):
    ...
    def configuremake_install_step(self):
        ...
        if cudaroot:
            ...
            if self.with_mpi:
                build_targets.append(("-cuda %s" % self.mpi_option, 'test.cuda.parallel'))
    ...
    def install_step(self):
        ...
        if self.cfg['runtest']:
            ...
            if self.with_cuda:
                res = run_shell_cmd("%s && make test.cuda.serial" % pretestcommands)
                if res.exit_code > 0:
            ...
            if self.with_mpi and self.with_cuda:
                ...
                res = run_shell_cmd("%s && make test.cuda.parallel" % pretestcommands)
                if res.exit_code > 0:
                    self.log.warning("Check the output of the Amber cuda.parallel tests for possible failures")
